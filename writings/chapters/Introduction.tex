...
%\lipsum[1]
	...
	% START MOTIVATION
	The goal of a secret key generation is, at the end, for Alice and Bob to share a \emph{common secret}.  A requirement is also that no eavesdropper Eve can know anything about this common secret.	
	...
	\subsubsection*{Diffie-Hellman key exchange}
		...
		\cite{4H07}
	\end{quotation}	
	A famous and widely used protocol for the exchange of cryptographic keys is the Diffie-Hellman key-exchange method.\footnote{ Diffie, W.; Hellman, M. (1976). "New directions in cryptography". IEEE Transactions on Information Theory. 22 (6): pp. 644–654.}
	...
	\begin{enumerate}
		...
		\item They mix their own secret with the common agreed basis, producing a result $R_a$ and $R_b$. The mathematical properties of this operation make it so it is computational infeasible to go back and retrieve the secrets $s$
		\item Both parties exchange publicly their result, so that they now posses the inseparable mixture secret-base of the other party.
		\item Each party applies again their secret but to the new mixture this time. The results are equal for Alice and Bob so they can use this result as a common secret to create a key
	\end{enumerate}	 
	The parts exchanged over the public channel are only the mutually agreed base and the two partial mixtures. It can be proven that those two elements alone give no information about the complete final shared secret and that it is virtually impossible to obtain the correct final product with only those two.\\
	This example shows how  the goal can be achieved for ALice and Bob starting from some common resource (in this case the common basis)

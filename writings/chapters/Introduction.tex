...
%\lipsum[1]
	...
	\subsubsection*{Diffie-Hellman key exchange}
		...
		\cite{4H07}
	\end{quotation}	
	A famous and widely used protocol for the exchange of cryptographic keys is the Diffie-Hellman key-exchange method.\footnote{ Diffie, W.; Hellman, M. (1976). "New directions in cryptography". IEEE Transactions on Information Theory. 22 (6): pp. 644–654.} %how cite this?
	...
	In this example the resource Alice and Bob start with could be seen as the \textit{basis} they publicly decide. %or the probability distribution on that?
	 ...
	 \begin{itemize}
		\item Is there a tripartite probability $P_{ABE}$ (\textsl{corresponding to the mixture above}), that has some \texttt{cost} associated to it to create it, but has $0$ possible key distillable from it? 
	 \end{itemize}
